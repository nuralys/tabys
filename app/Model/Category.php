...
class Category extends AppModel {
	...
	public function customUploadImg($file = array()){
		...
		$path = WWW_ROOT . 'img/category/' . $fileName;
		$path_th = WWW_ROOT . 'img/category/thumbs/' . $fileName;
		if(!move_uploaded_file($file['img']['tmp_name'], $path)){
			return false;
		}
		$this->resizeImg($path, $path_th, 200, 166, $ext);
	...
	public function genNameFile($ext){
		...
		if(is_file(WWW_ROOT . 'img/category/' . $name)){
